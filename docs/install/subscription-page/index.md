...
You can install subscription page on the same server as Remnawave Panel or on a different server. Select the appropriate guide below. For Kubernetes deployments, see the [Helm Chart subscription-page section](/install/helm-chart#subscription-page-sidecar) — the separate-Deployment topology is the K8s counterpart to the "Bundled" compose install.
